...
var strat = {
  ...
  init: function() {
    ...
    log.debug('Stop Loss: ' + this.settings.Stop_Loss_Percent);
  ...
  check: function(candle) {
    ...
    this.gain = (this.last_candle-this.lastLongPrice)/this.lastLongPrice*100

    if (this.gain < -this.settings.Stop_Loss_Percent){
    ...
    if (this.last_candle < this.lastLongPrice * (this.settings.Stop_Loss_Percent / 100)) {
    ...
    // BEAR TREND
      ...
      if (rsi > rsi_hi && rsi <= this.lastBearRsi) this.short(candle);
    ...
    // BULL TREND
      ...
      if (rsi > rsi_hi && rsi <= this.lastBullRsi) this.short(candle); // && RSI no longer increasing.
  ...
  long: function(candle) {
    if (this.trend.direction !== 'up') // new trend? (only act on new trends)
    {
      ...
      log.info('Long @ ' + this.lastLongPrice);
      if (this.debug) log.debug('Going long');
    }
  ...
  short: function(candle) {
    ...
    if (this.trend.direction !== 'down') {
      ...
      this.advice('short');
      this.last_candle = this.candle.close;
      this.state = 'WaitingBuy';
      log.info('Short @ ' + this.last_candle);
      if (this.debug) log.debug('Going short');
    }
  ...
  /* END backtest */
    ...
    if (this.debug) {
      let stat = this.stat;
      log.debug('BEAR RSI low/high: ' + stat.bear.min + ' / ' + stat.bear.max);
      log.debug('BULL RSI low/high: ' + stat.bull.min + ' / ' + stat.bull.max);
      log.debug('ADX min/max: ' + stat.adx.min + ' / ' + stat.adx.max);
    }
